...
const limiter = createRateLimiter({ windowMs: 60_000, max: 10 }, 'privacy-aprovar');
...
export async function POST(
  ...
  try {
    ...
    if (updateError) {
      logger.error('privacy_aprovar_update_error', { error: updateError.message });
    ...
    await supabase
      ...
      .insert({
        ...
        detalhes: { solicitacao_id: id, usuario_excluido: solicitacao.usuario_id },
      });
    ...
    await supabase
      ...
      .insert({
        ...
        mensagem: 'Sua solicitação de exclusão de dados foi aprovada. Sua conta será removida em instantes.',
        lida: false,
      });
    ...
    if (deleteAuthError) {
      logger.error('privacy_aprovar_delete_user_error', { error: deleteAuthError.message });
    ...
    logger.error('privacy_aprovar_error', { endpoint: '/api/privacy/[id]/aprovar' });
